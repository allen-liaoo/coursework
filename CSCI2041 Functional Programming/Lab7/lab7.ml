...
let mutyQueueEmpty q =
  match q with MutyQueueNode (_, left, right) -> 
    !left == q && !right == q;;

let mutyQueueEnqueue q e =
  match q with MutyQueueNode (_, left, _) ->
      match !left with MutyQueueNode (_, _, rofleft) ->
        left := MutyQueueNode(e, left, ref q);
        rofleft := !left;
        ();;

let mutyQueueDequeue q =
  match q with MutyQueueNode (_, _, right) ->
    match !right with MutyQueueNode (e, _, rofright) ->
      match !rofright with MutyQueueNode (_, lofrightright, _) ->
      right := !rofright;
      lofrightright := q;
      e;;
