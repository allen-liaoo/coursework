...
export class AnimatedBone extends gfx.Node3
{
    ...
    applyPose(pose: Pose): void
    {
        ...
        // this.boneSpaceToJointSpace = pose.getJointRotationMatrix(this.name)
        // this.jointSpaceToBoneSpace = this.boneSpaceToJointSpace.inverse()
        const locToPa = gfx.Matrix4.multiplyAll(
            this.boneSpaceToParentBoneSpace,    // convert child bone space to parent bone space
            this.jointSpaceToBoneSpace,         // convert back to bone space
            pose.getJointRotationMatrix(this.name),     // rotate the bone in joint space to fit the pose
            this.boneSpaceToJointSpace      // convert to joint space first
        )
        this.setLocalToParentMatrix(locToPa, false)
