...
void main() 
{
    ...
    vec3 tangentTBN = normalize((normalMatrix * vec4(tangent, 0)).xyz);
    vec3 normalTBN = normalize((normalMatrix * vec4(normal, 0)).xyz);
    tangentTBN = normalize(tangentTBN - dot(tangentTBN, normalTBN) * normalTBN);  // re-orthogonalize T with respect to N
    vec3 bitangentTBN = cross(normalTBN, tangentTBN);

    mat3 tbn = transpose(mat3(tangentTBN, bitangentTBN, normalTBN));
