...
export class Ground extends gfx.Mesh3
{
    ...
    public reshapeGround(stroke2D: Stroke2D, groundStartPoint: gfx.Vector3,  groundEndPoint: gfx.Vector3, camera: gfx.Camera): void
    {
        ...
        const diff = gfx.Vector3.subtract(groundEndPoint, groundStartPoint)
        const planeCenter = gfx.Vector3.add(groundStartPoint, gfx.Vector3.multiplyScalar(diff, 0.5))
        const planeNormal = gfx.Vector3.cross(gfx.Vector3.UP, diff)
        planeNormal.normalize()
        const projPlane = new gfx.Plane3(planeCenter, planeNormal)

        // project 2D stroke onto the projection plane to create a 3D stroke (silhouette curve, list of Vector3)
        const silhouetteCurve = []
        for (let i = 0; i < stroke2D.vertices.length; i++) {
            const v = stroke2D.vertices[i]
            const ray = new gfx.Ray3();
            ray.setPickRay(v, camera);
            const inter = ray.intersectsPlane(projPlane)
            if (inter) {
                silhouetteCurve.push(inter)
            }
        }

        // For each point on the ground, calculate how much we should raise it based 
        // on its distance to the projection plane
        for (let j = 0; j < this.vertices.length; j++) {
            const v = this.vertices[j]
            const proj = projPlane.project(v) // point of v projected to the plane
            const h = this.computeH(proj, silhouetteCurve, projPlane) // height of the ground at proj on the projection plane
            if (h === 0) continue
            // distance from v to proj
            const d = gfx.Vector3.distanceBetween(v, proj)
            const wd = Math.max(0, 1 - Math.pow(d/5,2))
            this.vertices[j].y = (1-wd) * v.y + wd * h
        }
