...
export class RainingApp extends gfx.GfxApp {
    ...
    // My variables
    private rdLastSpawn: number;
    private rdSpawnInterval: number; // in ms
    private readonly rdBaseSpawnInterval: number = 2000;
    ...
    constructor() {
        ...
        this.rdSpawnInterval = this.rdBaseSpawnInterval;
        this.rdLastSpawn = 0;
    ...
    update(deltaTime: number): void {
        ...
        if (latestImage instanceof ImageData) {
            ...
            if (this.displayImage instanceof ImageData && this.obstacleImage instanceof ImageData) {
                ...
                const now = Date.now();
                if (now - this.rdLastSpawn > this.rdSpawnInterval) {
                    this.rdSpawnInterval = this.rdBaseSpawnInterval + this.rdBaseSpawnInterval * (Math.random() * 0.5 - 1);
                    // randomly set next rain drop interval between rdBaseSpawnInterval +- 250 ms to achieve a more noisy rain
                    this.rdLastSpawn = now;
                    this.spawnRaindrop();
                }
                //====================================================================================================
                ...
                const raindropVelocity = -0.25; // distance in terms of pixels
                const deltaPos = raindropVelocity * deltaTime;
                for (const raindrop of this.raindropsParentNode.children) {
                    const rd = raindrop as gfx.Mesh2;
                    // move raindrop
                    rd.position.y += deltaPos;

                    // Check if raindrop is off screen
                    if (rd.position.y < -0.8) {
                        // console.log("deleted");
                        rd.remove();
                        continue;
                    }
                    ...
                    let col = this.sceneXtoImageColumn(rd.position.x, this.obstacleImage.width);
                    let row = this.sceneYtoImageRow(rd.position.y, this.obstacleImage.height);
                    
                    // console.log(`col:${col}, row:${row} converted from x:${rd.position.x}, y:${rd.position.y}`);
                    ...
                    // check if obstacle (if dark, obstacleImage have value of 0 in any r, g, b, or a value)
                    while(this.pixelIsDark(this.obstacleImage, col, row) && row > 0) {
                        ...
                        // move left or right if pixel arent dark
                        const dis = 4; // distance to look left or right
                        if (col < this.obstacleImage.width-dis && !this.pixelIsDark(this.obstacleImage, col+dis, row)) {
                            col += dis;
                            break;
                        } else if (col > 1 && !this.pixelIsDark(this.obstacleImage, col-dis, row)) {
                            col -= dis;
                            break;
                        }

                        // otherwise, move up until
                        row -= 2;
                    }

                    // Update position
                    rd.position.x = this.imageColumnToSceneX(col, this.obstacleImage.width);
                    rd.position.y = this.imageRowToSceneY(row, this.obstacleImage.height);
                }
    ...
    private spawnRaindrop(): void {
        ...
        const randInt = Math.floor(Math.random() * (this.wordArray.length-1));
        const randWord : string = this.wordArray[randInt];
        for (const ch of randWord) {
            const car = gfx.Geometry2Factory.createBox(0.1, 0.1);
            const randX = Math.random() * 2 - 1;  // between the range -1 to 1
            const randY = Math.random() * 0.4 + 0.8; // between the range 0.8 to 1.2
            car.position = new gfx.Vector2(randX, randY); // spawn across the x axis
            car.material.color = gfx.Color.BLACK;
            car.material.texture = new gfx.Text(ch, 64, 64, '48px Helvetica')//(ch, 50, 50);
            this.raindropsParentNode.add(car);
            // console.log(`raindrop ${ch} spawnned`);
        }
        //====================================================================================================
    }
    ...
    sceneXtoImageColumn(x: number, imageWidth: number) : number {
        const res = Math.floor((x+1) * imageWidth/2);
        return (res >= imageWidth ? res-1 : res);
        // // I spent to long thinking of a non-conditional formula to calculate this without the index going out of bounds
        // // I give up :(
    }

    sceneYtoImageRow(y: number, imageHeight: number): number  {
        const res =  Math.floor((1-y) * imageHeight/2);
        return (res >= imageHeight ? res-1 : res);
    }

    imageColumnToSceneX(col: number, imageWidth: number): number  {
        return (col - imageWidth/2)/(imageWidth/2);
    }

    imageRowToSceneY(row: number, imageHeight: number): number  {
        return (imageHeight/2 - row)/(imageHeight/2);
    }

    pixelIsDark(image: ImageData, col: number, row: number) {
        return ImageUtils.getRed(image, col, row) == 0;
    }
    //====================================================================================================
}